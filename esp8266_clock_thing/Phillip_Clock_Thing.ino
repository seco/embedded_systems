...
// 2 occasional LED dot matrix module blanking was observed, have added an
//   LedCont_reset() to resync as check
...
unsigned long sendNTPpacket(IPAddress &address) {
  ...
  udp.beginPacket(address, 123);// NTP request on port 123
...
void handleUpdate() {
  ...
  delay(5000);
  clear_lcd_row(3);
  lcd.print("Please cycle power");      // should never reach here...
}
...
void WIFIap_loop() { server.handleClient(); }
...
void set_clock_part2() {
  ...
  if (!cb) {
    ...
    tmp_ms = (dt_mins * 60 + dt_secs) - tmp_ms; // estimate(!) correction
                                                // measurement (assume error is
                                                // a few seconds)
    if (tmp_ms > 3600)
      tmp_ms = tmp_ms - 3600;
    if (tmp_ms >= 0)
...
void setup() {
  ...
  clear_lcd_row(2); lcd.print( "2016_v1" );
